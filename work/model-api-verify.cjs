...
const root=path.resolve('outputs/video-notes/3.认识模型服务接口'),share=path.join(root,'share'),out=path.join(root,'verify/deck-pages');fs.mkdirSync(out,{recursive:true});
(async()=>{
...
await page.pdf({path:path.join(share,'3.认识模型服务接口-visual-explainer.pdf'),format:'A4',printBackground:true,preferCSSPageSize:true});
