...
import ast,base64,html,io,json,re,subprocess,sys
from fontTools import subset
root=Path(__file__).resolve().parents[1]
p=root/'outputs/video-notes/3.认识模型服务接口';share=p/'share';share.mkdir(exist_ok=True)
...
for i,s in enumerate(slides,1):
 ...
 sections.append(f'<section class="slide {s.get("kind","")}'+(' active visible' if i==1 else '')+f'" aria-label="第{i}页"><div class="eyebrow">AI 基础 / 第三课</div><h2 class="reveal" data-edit>{s["title"]}</h2><div class="content reveal">{body}</div><div class="footer"><a class="source-link" href="完整教程.html#section-{s["scene"]:02}" target="_blank">阅读完整讲解 ↗</a><span>MODEL SERVICE / {i:02}</span></div><aside class="speaker-notes">{speaker}</aside></section>')
controller=(root/'work/raw-model-deck-controller.js').read_text(encoding='utf-8').replace('raw-model-paper-ink-v1','model-api-paper-ink-v1').replace('Raw-Model-重点演示稿.html','模型服务接口-重点演示稿.html')
deck='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>认识模型服务接口 · 重点演示稿</title><style>FONT_CSS\n'+base+css+'</style></head><body><div class="deck-viewport"><main class="deck-stage">'+''.join(sections)+'''</main></div><nav class="deck-controls" aria-label="演示控制"><button id="prev" aria-label="上一页">←</button><span id="pageCount"></span><button id="next" aria-label="下一页">→</button><button id="notesToggle">讲者备注 N</button><button id="save">保存</button><a href="完整教程.html" target="_blank">完整教程 ↗</a></nav><aside id="notes" hidden><button id="closeNotes" aria-label="关闭备注">×</button><div id="notesContent"></div></aside><div class="edit-hotzone"></div><button class="edit-toggle" id="editToggle">编辑 E</button><div id="progress"></div><script>'''+controller+'</script></body></html>'
...
article_css+='@media print{#section-13 figure img{max-height:58mm}#section-13 p{font-size:10.7px;line-height:1.7;margin:7px 0}#section-13 pre{font-size:9px;line-height:1.4;padding:9px}#section-13 .visual p{font-size:9px}}'
article='<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>认识模型服务接口 · 完整教程</title><style>FONT_CSS\n'+article_css+'</style></head><body><header><div class="kicker">AI 基础 · 第三课</div><h1>认识模型服务接口</h1><p>从 API 调用走进模型服务内部，理解预处理、自回归生成、采样参数与后处理。保留课堂讲解、案例和代码推导。</p><div class="actions"><a href="重点演示稿.html">重点演示稿 ↗</a><a href="3.认识模型服务接口-visual-explainer.pdf">PDF 讲义 ↗</a></div></header><div class="layout"><nav aria-label="章节目录">'+''.join(f'<a href="#section-{n["id"]:02}">{n["id"]:02} · {esc(n["title"])}</a>' for n in notes)+'</nav><article>'
for n,s in zip(notes,manifest['scenes']):
 article+=f'<section id="section-{n["id"]:02}"><h2>{n["id"]:02} · {esc(n["title"])}</h2><figure><img src="keyframes/{Path(s["frame_path"]).name}" alt="{esc(n["title"])}课堂画面"><figcaption>课堂画面 · 点击放大</figcaption></figure>'+md(n['body'])+'<aside class="visual"><h3>画面与校读说明</h3>'+md(n['visual'])+'</aside></section>'
article+='</article></div><footer class="source-note"><p>来源：用户提供的《3.认识模型服务接口.mp4》。正文依照课堂顺序轻度整理；课程中的价格、产品界面和参数说明保留录制时的语境，未作为当前产品规格核验。</p><p>本地 Qwen3-ASR 0.6B INT8 转写，结合画面核对术语；原始音频块保存在 source-materials。时间仅为音频块粒度，不是逐词对齐；本次未逐字人工听校，可能仍有细小识别误差。实际模型 Token 用量与费用未由运行环境提供。</p></footer><dialog id="zoom"><button class="close" onclick="this.parentElement.close()">关闭 ×</button><img alt="放大画面"></dialog><script>document.querySelectorAll("figure img").forEach(im=>im.onclick=()=>{let d=document.querySelector("#zoom");d.querySelector("img").src=im.src;d.showModal()});document.querySelector("#zoom").addEventListener("click",e=>{if(e.target.tagName==="DIALOG")e.target.close()})</script></body></html>'
